# Pixel Drive Capture & AI Edit Image / Video (Gemini & ChatGPT Helper)
...
3. **Khởi chạy**: Click đúp tệp **`PixelDriveCapture.exe`** để chạy trực tiếp Flask server. Các tệp cấu hình (`config.json`, `content_prompts.json`, `run_debug_chrome.bat`, `run_debug_chrome_gemini.bat`) sẽ tự động được sinh ra ở ngoài thư mục root để bạn dễ dàng chỉnh sửa cấu hình.
