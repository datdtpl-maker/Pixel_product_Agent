...
3. **Khởi chạy**: Click đúp tệp **`PixelDriveCapture.exe`** để chạy trực tiếp Flask server. Các tệp cấu hình (`config.json`, `content_prompts.json`, `run_debug_chrome.bat`) sẽ tự động được sinh ra ở ngoài thư mục root để bạn dễ dàng chỉnh sửa cấu hình.
...
1. **Hiển thị phiên bản**: Phiên bản hiện tại của phần mềm được hiển thị trên nút ở góc trên bên phải (ví dụ: `v1.1.16`).
...
3. **Quy trình cập nhật cài đè an toàn nâng cao (v1.1.16+)**: 
   ...
   - Tiến trình `updater.bat` sẽ liên tục kiểm tra trạng thái tiến trình hệ thống bằng lệnh `tasklist` và sử dụng lệnh `ping` tạo khoảng trễ để chờ cho đến khi tệp `PixelDriveCapture.exe` cũ tắt hẳn và các tệp DLL trong thư mục `_internal` được giải phóng hoàn toàn.
